...
(function () {
  ...
  const CORE_JS = '//www.mediawiki.org/w/index.php?title=User:Iniquity/scriptManager-core.js&action=raw&ctype=text/javascript'
  const CORE_CSS = '//www.mediawiki.org/w/index.php?title=User:Iniquity/scriptManager-core.css&action=raw&ctype=text/css'
  ...
  function loadSidebarMessages (callback) {
    try {
      const langs = []
      if (USER_LANG) {
        langs.push(USER_LANG); const base = USER_LANG.split('-')[0]; if (base && base !== USER_LANG) langs.push(base) 
      }
      if (langs.indexOf('en') === -1) langs.push('en')
      let idx = 0
      function tryNext () {
        if (idx >= langs.length) {
          if (callback) callback({ label: 'Script Manager', title: 'Script Manager' }); return 
        }
        const lang = langs[idx++]
        const url = 'https://gitlab-content.toolforge.org/iniquity/script-manager/-/raw/main/i18n/' + encodeURIComponent(lang) + '.json?mime=application/json'
        fetch(url).then(function (r) {
          ...
          try {
            ...
            if (label) {
              if (callback) callback({ label, title, capturedScriptsHeading: cap }); return 
            }
          } catch (_) {}
          tryNext()
        }).catch(function () {
  ...
  function loadCssOnce () {
    ...
    try {
      mw.loader.load(CORE_CSS, 'text/css'); window.__SM_CSS_LOADED = true 
    } catch (_) {
      const el = document.createElement('link'); el.rel='stylesheet'; el.href=CORE_CSS; document.head.appendChild(el)
  ...
  function ensureCoreLoaded (callback) {
    ...
    if (mw.loader && typeof mw.loader.getScript === 'function') {
      ...
      const s = document.createElement('script'); s.src = CORE_JS; s.async = true; s.onload = done; document.head.appendChild(s)
    }
  }
  ...
  (function () {
    ...
    function isCss (spec, type) {
      ...
      try {
        const s = String(spec||''); if (/ctype=text\/css/i.test(s)) return true; if (/\.css(\?|$)/i.test(s)) return true 
      } catch (_) {}
      return false
    }
    function execCaptured (item) {
      try {
        ...
        if (item.callType === 'mw.loader.getScript') {
          ...
          (ORIG.getScript || (mw && mw.loader && mw.loader.getScript) || function () {
        ...
        if (item.callType === 'importStylesheet') {
          capLog('exec: importStylesheet', item.args && item.args[0]); (ORIG.importStylesheet || window.importStylesheet)(item.args && item.args[0]); return 
        }
        if (item.callType === 'importStylesheetURI') {
          capLog('exec: importStylesheetURI', item.args && item.args[0]); (ORIG.importStylesheetURI || window.importStylesheetURI)(item.args && item.args[0]); return 
        }
      } catch (_) {}
    }
    function renderCaptured (items) {
      try {
        ...
        if (!tb) {
          capLog('render: #p-tb not found'); return 
        }
        // Remove previous capture subsection if any
        try {
          const oldHeading = document.getElementById('smcap-heading'); if (oldHeading) oldHeading.remove()
          const oldContent = document.getElementById('smcap-content'); if (oldContent) oldContent.remove()
          const oldLis = tb.querySelectorAll('li[id^="n-smcap-"]'); for (let i=0;i<oldLis.length;i++) {
            oldLis[i].remove() 
          }
        } catch (_) {}
        if (!items || !items.length) {
          capLog('render: empty list'); return 
        }
        ...
        items.forEach(function (it, idx) {
          ...
          a.addEventListener('click', function (ev) {
            ev.preventDefault(); capLog('click:', it); execCaptured(it) 
          })
          li.appendChild(a)
          ul.appendChild(li)
        })
    ...
    function dedupe (list) {
      ...
      for (let i=0;i<list.length;i++) {
        const it = list[i]; const k = (it.callType||'')+'\u0001'+(it.key||'')+'\u0001'+(it.isCss?1:0)
    ...
    function captureLoads (runFn) {
      ...
      try {
        if (!ORIG.importStylesheet && typeof window.importStylesheet === 'function') ORIG.importStylesheet = window.importStylesheet 
      } catch (_) {}
      try {
        if (!ORIG.importStylesheetURI && typeof window.importStylesheetURI === 'function') ORIG.importStylesheetURI = window.importStylesheetURI 
      } catch (_) {}
      ...
      function patchedGetScript (url) {
        capLog('capture: mw.loader.getScript', url); try {
          const u = String(url||''); captured.push({ callType:'mw.loader.getScript', args:[u], isCss:false, key:u, label: labelFromUrl(u) }) 
        } catch (_) {} return Promise.resolve() 
      }
      function patchedImportScript (title) {
        capLog('capture: importScript', title); try {
          const t=String(title||''); captured.push({ callType:'importScript', args:[t], isCss:false, key:t, label:t }) 
        } catch (_) {} 
      }
      function patchedImportStylesheet (title) {
        capLog('capture: importStylesheet', title); try {
          const t=String(title||''); captured.push({ callType:'importStylesheet', args:[t], isCss:true, key:t, label:t }) 
        } catch (_) {} 
      }
      function patchedImportStylesheetURI (u) {
        capLog('capture: importStylesheetURI', u); try {
          const s=String(u||''); captured.push({ callType:'importStylesheetURI', args:[s], isCss:true, key:s, label: labelFromUrl(s) }) 
        } catch (_) {} 
      }
      // patch
      try {
        if (mw && mw.loader) {
          mw.loader.load = patchedLoad; if (typeof mw.loader.getScript === 'function') mw.loader.getScript = patchedGetScript 
        } 
      } catch (_) {}
      try {
      ...
      try {
        window.__SM_LAST_CAPTURED = items; capLog('capture: done, items=', items.length) 
      } catch (_) {}
      ...
      try {
        if (mw && mw.hook && typeof mw.hook === 'function') {
          mw.hook('scriptManager.capture').add(function (payload) {
            const p = norm(payload); if (p) captureLoads(p.fn) 
          }) 
        } 
      } catch (_) {}
      try {
        document.addEventListener('sm:capture', function (ev) {
          try {
            const d = ev && ev.detail; const p = norm(d); if (p) captureLoads(p.fn) 
          } catch (_) {} 
        }) 
      } catch (_) {}
    }
        ...
        if (!link) {
          ...
          link.addEventListener('click', function (e) {
            ...
            ensureCoreLoaded(function () {
              try {
                if (mw && mw.hook) {
                  mw.hook('scriptManager.open').fire(); return 
                }
              } catch (_) {}
              try {
          ...
          try {
            link.textContent = label; link.setAttribute('title', title) 
          } catch (_) {}
        }
