...
;(function() {
	...
	function getCapturedHeading() {
		try {
			const raw = localStorage.getItem(SIDEBAR_I18N_CACHE_KEY);
			if (!raw) return DEFAULT_CAPTURED_HEADING;
			const cached = JSON.parse(raw);
			const data = cached && cached.data;
			if (data && data.capturedScriptsHeading) {
				return data.capturedScriptsHeading;
			}
		} catch {}
		return DEFAULT_CAPTURED_HEADING;
	}
	...
	function renderCaptured(items) {
		try {
			...
			heading.textContent = getCapturedHeading();
