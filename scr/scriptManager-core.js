...
;(function() {
	...
	const SM_LOG_LEVEL_VAR = (function() {
		try {
			return (window.SM_LOG_LEVEL || (window.scriptInstallerDebug ? 'debug' : 'info') || 'info')
				.toString()
				.toLowerCase();
		} catch {
			return 'info';
	...
	function canonicalizeUserNamespace(title) {
		try {
			...
			smLog('canonicalizeUserNamespace failed', e);
			return title;
		}
	}
	...
	function smRenderIconInto(el, iconName, colorHex, sizePx) {
		...
		try {
			const cm = localStorage.getItem(keyMarkup);
			if (cm) {
				el.innerHTML = cm;
				return;
			}
	...
	function applyGadgetLabels(sectionLabels, gadgetsLabel) {
		...
		smInfo(
			'applyGadgetLabels: input sections =',
			Object.keys(gadgetSectionLabelsVar || {}),
			'gadgetsLabel =',
			gadgetsLabelVar
		);
		try {
	...
	function getTargetsForScript(name) {
		try {
			...
			smLog('getTargetsForScript failed', e);
			return [];
		}
	}
	...
	function resolveDocumentationInterwiki(imp) {
		try {
			...
			const rawUrl =
				'//' + host + '/w/index.php?title=' + encodeURIComponent(title) + '&action=raw&ctype=text/javascript';
			return fetch(rawUrl)
				.then(function(r) {
	...
	createImport.prototype.toJs = function() {
		...
		const url = this.type === 2 ? this.url : buildRawLoaderUrl(host, title);
		...
		const suffix = this.type === 2 ? '' : ' // ' + backlinkText + ' [[' + escapeForJsComment(backlinkPage) + ']]';
	...
	createImport.prototype.uninstall = function(options) {
		...
		const chain = resolveDocumentationInterwiki(this)
			.then(function(iw) {
				if (iw) {
					try {
	...
	createImport.prototype.setDisabled = function(disabled) {
		const that = this;
		this.disabled = disabled;
		return resolveDocumentationInterwiki(this)
			.then(function(iw) {
				if (iw) {
					try {
						that.docInterwiki = iw;
	...
	function getAllTargetWikitexts() {
		...
		return $.when(localPromise, globalPromise)
			.then(function(localData, globalData) {
				const result = {};
				...
				const localPages =
					localData && localData.query && localData.query.pages
						? localData.query.pages
						: localData && localData[0] && localData[0].query && localData[0].query.pages
							? localData[0].query.pages
							: null;
				const globalPages =
					globalData && globalData.query && globalData.query.pages
						? globalData.query.pages
						: globalData && globalData[0] && globalData[0].query && globalData[0].query.pages
							? globalData[0].query.pages
							: null;
	...
	function loadGadgets() {
		...
		_pLoadGadgets = api
			...
			.then(function(data) {
				if (data && data.query && data.query.gadgets) {
					...
					data.query.gadgets.forEach(function(gadget) {
						...
						const isDefault = gadget.metadata && gadget.metadata.settings && gadget.metadata.settings.default === '';
			...
			.always(function() {
				...
				smInfo('loadGadgets: done in', Math.round(t1 - t0), 'ms');
				_pLoadGadgets = null;
			});
	...
	function loadGadgetsLabel() {
		...
		return api
			...
			.then(function(msgData) {
				...
				if (
					msgData.query &&
					msgData.query.allmessages &&
					msgData.query.allmessages[0] &&
					msgData.query.allmessages[0]['*']
				) {
	...
	function loadSectionLabels() {
		...
		const names = Array.from(sections)
			.filter(function(s) {
				return s && s !== 'other';
			})
			.map(function(s) {
		...
		return api
			...
			.always(function() {
				const t1 = typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now();
				smLog('loadSectionLabels: done in', Math.round(t1 - t0), 'ms');
			});
	}
	...
	function normalize(target) {
		return getWikitext(getFullTarget(target))
			.then(function(wikitext) {
	...
	function showNotification(messageKeyOrText, type, param) {
		...
		if (typeof messageKeyOrText === 'string') {
			...
			const localized =
				typeof t === 'function'
					? t(messageKeyOrText)
					: typeof SM_t === 'function'
						? SM_t(messageKeyOrText)
						: messageKeyOrText;
	...
	function createVuePanel(
		...
		const ScriptManager = defineComponent({
			...
			setup() {
				...
				try {
					gadgetSectionLabelsRef = gadgetSectionLabels;
					gadgetsLabelRef = gadgetsLabel;
				} catch {}
				...
				const filteredImports = computed(function() {
					...
					try {
						void gadgetSectionLabels.value;
						void gadgetsLabel.value;
					} catch {}
					const result = {};
					...
					if (selectedSkin.value === 'gadgets') {
						...
						sortedSections.forEach(function(section) {
							...
							const sortedGadgets = {};
							Object.keys(sectionGadgets)
								.sort()
								.forEach(function(gadgetName) {
				...
				const handleToggleDisabled = function(anImport) {
					try {
						...
						if (!(anImport instanceof createImport) || typeof anImport.toggleDisabled !== 'function') {
							...
							anImport = url
								? createImport.ofUrl(url, target, disabled)
								: wiki
									? new createImport(page, wiki, null, target, disabled)
									: createImport.ofLocal(page, target, disabled);
						}
					} catch {}
				...
				const handleNormalizeAll = function() {
					...
					Promise.all(normalizePromises)
						...
						? Promise.all(normalizePromises).finally(function() {
				...
				const handleGadgetToggle = function(gadgetName, enabled) {
					...
					toggleGadget(gadgetName, enabled)
						.then(function() {
			...
			try {
				;(typeof requestAnimationFrame === 'function' ? requestAnimationFrame : setTimeout)(function() {
					try {
						...
						if (
							comp &&
							comp.gadgetSectionLabels &&
							typeof comp.gadgetSectionLabels === 'object' &&
							'value' in comp.gadgetSectionLabels
						) {
	...
	function buildCurrentPageInstallElement() {
		...
		if (namespaceNumber === SM_USER_NAMESPACE_NUMBER && pageName.indexOf('/') > 0) {
			...
			if (isCodeModel) {
				...
				if (pageName.indexOf(USER_NAMESPACE_NAME + ':' + mw.config.get('wgUserName')) === 0) {
					...
					if (skinIndex >= 0) {
						return $('<abbr>').text(SM_t('error-cannot-install')).attr('title', SM_t('error-cannot-install-skin'));
					}
				}
				addingInstallLink = true;
			} else {
				return $('<abbr>')
					.text(SM_t('error-cannot-install') + ' (' + SM_t('error-not-javascript') + ')')
		...
		if (namespaceNumber === SM_MEDIAWIKI_NAMESPACE_NUMBER) {
			return $('<a>')
				.text(SM_t('error-install-via-preferences'))
		...
		if (
			...
			(editRestriction.indexOf('sysop') >= 0 || editRestriction.indexOf('editprotected') >= 0)
		) {
		...
		if (addingInstallLink) {
			...
			const allScriptsInTarget =
				importsRef && importsRef.value ? importsRef.value[installedTargets] : imports[installedTargets];
			const importObj =
				allScriptsInTarget &&
		...
		return $('<abbr>')
			.text(SM_t('error-cannot-install') + ' ' + SM_t('error-insecure'))
	...
	function showUi() {
		if (!document.getElementById('sm-top-container')) {
			const fixedPageName = mw.config.get('wgPageName').replace(/_/g, ' ');
			try {
				const sub = document.getElementById('contentSub');
				if (sub) sub.classList.add('sm-contentSub');
			} catch {}
			$('#firstHeading').append(
				$('<span>')
			...
			function injectInstallIndicator() {
				try {
					...
					smLog('mw-indicators injection failed', e);
					return false;
				}
			}
				...
				const app = libs.createApp({
					...
					methods: {
						onClick() {
							...
							if (vm.busy) return;
							vm.busy = true;
							smInfo('install button set busy=true');
							if (vm.label === SM_t('action-install')) {
								const adapter = {
									text(t) {
										try {
											vm.label = String(t);
											smLog('adapter.text set label', t);
										} catch {}
									},
									resetBusy() {
										try {
											vm.busy = false;
											smLog('adapter.resetBusy executed');
										} catch {}
									}
								};
								try {
									smInfo('opening install dialog for', scriptName);
									showInstallDialog(scriptName, adapter);
								} catch (e) {
									vm.busy = false;
									smError('showInstallDialog error', e);
								}
							} else {
								vm.label = SM_t('action-uninstall-progress');
								smInfo('uninstall start (from infobox button)', {
									scriptName,
									targets: getTargetsForScript(scriptName)
								});
								...
								Promise.all(uninstalls)
									.then(function() {
										smInfo('uninstall via button success', { scriptName, targets });
										vm.label = SM_t('action-install');
										return refreshImportsView();
									})
									.catch(function(e) {
										smError('uninstall via button failed', e);
									}).finally
									? Promise.all(uninstalls).finally(function() {
										vm.busy = false;
									})
									: (function() {
										vm.busy = false;
		...
		$('table.infobox-user-script').each(function() {
			...
			if (!scriptName) {
				try {
					...
					const $th = $table
						.find("th:contains('Source'), th:contains('Исход'), th:contains('Источник'), th:contains('Исходник')")
						.first();
					if ($th.length) {
			...
			try {
				const m2 = /user:.+?\/.+?\.js/i.exec(scriptName);
				if (m2) {
					scriptName = m2[0];
				}
			} catch {}
					...
					try {
						if (!document.getElementById('sm-install-dialog')) {
							if (buttonElement && typeof buttonElement.resetBusy === 'function') {
								...
								smLog('observer: resetBusy after dialog removal');
							}
			...
			loadVueCodex()
				...
				.catch(function(error) {
					...
					const okay = window.confirm(
						SM_t('security-warning').replace('$1', SM_t('security-warning-section').replace('$1', scriptName))
					);
					if (okay) {
	...
	function getFullTarget(target) {
		...
		return USER_NAMESPACE_NAME + ':' + mw.config.get('wgUserName') + '/' + target + '.js';
	}
	...
	function getSummaryForTarget(target, summaryKey, description) {
		try {
			...
			if (target === 'global' || englishOnlyHost) {
				return (
					(STRINGS_EN[summaryKey] || summaryKey).replace('$1', description) + (SUMMARY_TAG ? ' ' + SUMMARY_TAG : '')
				);
			}
	...
	fetch(
		'https://gitlab-content.toolforge.org/iniquity/script-manager/-/raw/main/data/languageFallbacks.json?mime=application/json'
	)
		.then(resp => resp.json())
		.then(fallbacks => {
	...
	function t(key) {
		try {
			...
			if (STRINGS_EN && Object.prototype.hasOwnProperty.call(STRINGS_EN, key)) {
				smWarn('Missing i18n key in current language:', key);
				return STRINGS_EN[key];
			}
			smWarn('Missing i18n key in all languages:', key);
			return key;
		} catch {
			return key;
		}
	...
	try {
		...
		try {
			...
			if (!SHOULD_PREWARM && (document.getElementsByClassName('scriptInstallerLink').length > 0 || ns === 2))
				SHOULD_PREWARM = true;
		} catch {}
		...
		if (!SM_GADGETS_READY && !SM_GADGETS_LOADING) {
			...
			loadGadgets()
				...
				.then(function(sectionLabels) {
					applyGadgetLabels(sectionLabels, gadgetsLabelVar);
					SM_GADGETS_READY = true;
				})
				.catch(function() {
					SM_GADGETS_READY = true;
	...
	loadI18nWithFallback(userLang, function() {
		...
		$.when($.ready, mw.loader.using([ 'mediawiki.api', 'mediawiki.ForeignApi', 'mediawiki.util' ]))
			.then(function() {
				api = new mw.Api();
				metaApi = new mw.ForeignApi('https://meta.wikimedia.org/w/api.php');
				try {
					SM_API_READY = true
					;(__SM_apiCbs || []).splice(0).forEach(function(cb) {
						try {
							cb();
						} catch {}
					});
				...
				buildImportList([ initialTarget ]).then(function() {
					try {
						SM_IMPORTS_READY = true
						;(__SM_importCbs || []).splice(0).forEach(function(cb) {
							try {
								cb();
							} catch {}
						});
	...
	try {
		...
		try {
			if (mw && mw.hook)
				mw.hook('scriptManager.open').add(function() {
